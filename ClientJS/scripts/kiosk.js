$(function () {
	...
	$('#signout').click(function (e) {
		...
		$("#display-success").append(
			"<div class='alert alert-success alert-dismissable' ><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>You're Signed out!</strong> </div>"
		);
		e.preventDefault();
	}); //Login submit button click
	...
	$('#register-submit').click(function (e) {
		...
		if (validateSignupForm(firstName, lastName, email, password, passwordConfirm)) {
			...
			if (signupStaus != -1) {
				$("#display-success").append(
					"<div class='alert alert-success alert-dismissable' ><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>You Signed up with Success!</strong> You may login now.</div>"
				);
